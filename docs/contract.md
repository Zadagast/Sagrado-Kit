...
17. **Emoji Picker** — Ubuntu Characters–style IA in Sagrado gel (`paint_emoji_picker`): title **Emoji**, search field (`paint_field`), left rail (Recently Used + categories), 6-column scrollable grid of 48px glyphs, Cancel + gel X. Category labels are ASCII (kit font is Latin-1); glyphs come from an **on-disk Noto Color Emoji pack** (`build/emoji_pack/` via `make emoji-pack` — `catalog.txt` + `png48/` / `png32/`), not embedded headers. Apps call `emoji_pack_set_root` then host the dialog; `emoji_icon` / `paint_emoji_marks` blit sharp PNGs into other surfaces. No new Hap art slots — compose from existing kit paint.
