...
int main(int argc, char **argv) {
    ...
    bool gamespot = false, ooze = false, ashen = false;
    ...
    std::printf("bundled=%d gamespot=%d ooze=%d ashen=%d default=%s\n",
                (int)skins.size(), gamespot ? 1 : 0, ooze ? 1 : 0, ashen ? 1 : 0,
                def.c_str());
    if ((int)skins.size() < 50) return 1;
    if (!gamespot || !ooze || !ashen) return 2;
